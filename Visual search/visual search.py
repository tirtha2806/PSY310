...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\91931\\Desktop\\visual search.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixtion = visual.ShapeStim(
        win=win, name='fixtion', vertices='cross',
        size=(0.1, 0.1),
    ...
    trial = data.Routine(
        name='trial',
        components=[fixtion, target, distr],
    )
    ...
    while continueRoutine:
        ...
        # if fixtion is starting this frame...
        if fixtion.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            fixtion.frameNStart = frameN  # exact frame index
            fixtion.tStart = t  # local t and not account for scr refresh
            fixtion.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fixtion, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'fixtion.started')
            # update status
            fixtion.status = STARTED
            fixtion.setAutoDraw(True)
        
        # if fixtion is active this frame...
        if fixtion.status == STARTED:
            # update params
            pass
        
        # if fixtion is stopping this frame...
        if fixtion.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fixtion.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                fixtion.tStop = t  # not accounting for scr refresh
                fixtion.tStopRefresh = tThisFlipGlobal  # on global time
                fixtion.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fixtion.stopped')
                # update status
                fixtion.status = FINISHED
